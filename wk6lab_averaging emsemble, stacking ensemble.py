...
#     for i in range(0, NUM_MODELS):    #개별 모델별
#         model                   = fitModel(trainX, trainy)
#         models.append(model)
#         predictions             = model.predict(testX)
#         yhats.append(predictions)  #categorical predictions
#
...
#         for model in models:   #위에서 이미 구축한 모델들로
#             predictions = model.predict(testX)
...
#         summed = np.sum(yhats, axis=0)   #여기서 judge들의 vote 볼 수 있음.
#
#         # Converts multi-column prediction set back to single column
...
#
#         accuracy = accuracy_score(singleColumnPredictions, testy)  #accuracy 외에도 다양히 구할 수 있음
#         scores.append(accuracy)
...
def getStackedData(models, inputX):
    stackXdf = None   #디버거해보든가
    for model in models:   #5모델을 돌며, 각 모델이 validation data 갖고 prediction함.
        # make prediction
        yhat = model.predict(inputX, verbose=0) #각 모델이 validation data 갖고 prediction함
        singleModelPredDf = pd.DataFrame(np.row_stack(yhat))  #각 output활용, 각 클래스의 확률. Yhat의 첫번째는 가운데 걸로 판정할 거임.
    ...
    return stackXdf   # 5 models * 3 classes 해서 15개 column 나옴..
...
def fit_stacked_model(models, inputX, inputy):
    # create dataset using ensemble
    stackedX = getStackedData(models, inputX)   #5모델과 validation data를 넘겨주는중
    # fit standalone model
    model = LogisticRegression()  #뭐든지 원하는대로.
